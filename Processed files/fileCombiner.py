...
specialFiles = ['Final_Processed_Data_FoodInfo.csv',
                'Final_Processed_Data_FoodFrequency.csv', 'combinedFiles.csv', 'finalCombinedFiles.csv']
...
outputFile = pd.read_csv(result[0])

for i in range(len(result[1:])):
    inputFile = pd.read_csv(result[i])
...
df.to_csv("finalCombinedFiles.csv")