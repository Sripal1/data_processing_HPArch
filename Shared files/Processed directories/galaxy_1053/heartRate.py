...
fileToRead = "com.samsung.shealth.tracker.heart_rate.202112041053.csv"
dataType = "HeartRate"
...
for header in usefulColumns:
    ...
    for item in toBeRemovedHeartRate:
        if header.strip() == item:
            obsoleteColumns.append(header.strip())
            usefulColumns.remove(header.strip())
obsoleteColumns.append("com.samsung.health.heart_rate.heart_beat_count")
usefulColumns.remove("com.samsung.health.heart_rate.heart_beat_count")
obsoleteColumns.append("source")
usefulColumns.remove("source")
