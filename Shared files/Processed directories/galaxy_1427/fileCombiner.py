...
for i in range(len(newResult[1:])):
    ...
    # inputFile['Date'] = inputFile['Date'].astype(str)
    # outputFile['Date'] = outputFile['Date'].astype(str)
...
df.to_csv("galaxy_1427_CombinedFiles.csv")

with open('galaxy_1427_CombinedFiles.csv', 'a', newline='') as f_object:
