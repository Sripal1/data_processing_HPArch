...
fileToRead = "com.samsung.shealth.tracker.heart_rate.202112181156.csv"
dataType = "HeartRate"
...
for item in dayList:
    ...
    for dateData in item:
        if minDayHR == None:
            minDayHR = float(dateData[-1].strip())
        elif float(dateData[-1].strip()) < minDayHR:
            minDayHR = float(dateData[-1].strip())

        if maxDayHR == None:
            maxDayHR = float(dateData[-1].strip())
        elif float(dateData[-1].strip()) > maxDayHR:
            maxDayHR = float(dateData[-1].strip())
    ...
    dayDict[dateData[0].strip()]={"Avg. day HR":round(avgDayHR,2),"Max. average HR (day)": maxDayHR,"Min. average HR (day)":minDayHR,"Number of samples (day)":counter}
# print(dayDict)
...
for item in nightList:
    ...
    for dateData in item:
        # print(dateData)
        if minNightHR == None:
            minNightHR = float(dateData[-1].strip())
        elif float(dateData[-1].strip()) < minNightHR:
            minNightHR = float(dateData[-1].strip())

        if maxNightHR == None:
            maxNightHR = float(dateData[-1].strip())
        elif float(dateData[-1].strip()) > maxNightHR:
            maxNightHR = float(dateData[-1].strip())
    ...
    nightDict[dateData[0].strip()]={"Avg. night HR":round(avgNightHR,2),"Max. average HR (night)": maxNightHR,"Min. average HR (night)":minNightHR,"Number of samples (night)":counter}
# print(nightDict)
...
for item in dateGroupedList:
    ...
    for data in item:
        if dailyMinHR == None:
            dailyMinHR = float(data[-1].strip())
        elif float(data[-1].strip()) < dailyMinHR:
            dailyMinHR = float(data[-1].strip())

        if dailyMaxHR == None:
            dailyMaxHR = float(data[-1].strip())
        elif float(data[-1].strip()) > dailyMaxHR :
            dailyMaxHR  = float(data[-1].strip())
        avgHR += float(data[-1].strip())
        counter+=1
    avgHR=avgHR/counter
    dailyDict[data[0]] = {"Min. average HR (daily)": round(dailyMinHR,2), "Max. average HR (daily)": round(dailyMaxHR, 2),
