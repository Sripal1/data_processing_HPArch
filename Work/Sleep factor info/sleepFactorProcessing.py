...
for item in dateGroupedList:
    ...
    for data in item:
        try:
            avgMentalRecovery += (float(data[0].strip()))*(float(data[-3].strip()))
        except:
            avgMentalRecovery+=0.0
        try:
            avgPhysicalRecovery += (float(data[1].strip())) * \
                (float(data[-3].strip()))
        except:
            avgPhysicalRecovery+=0.0
        try:
            avgMovementAwakening += (float(data[2].strip())) * \
                (float(data[-3].strip()))
        except:
            avgMovementAwakening+=0.0
        try:
            totalSleepCycles += float(data[3].strip())
        except:
            totalSleepCycles+=0.0
        try:
    ...
    dictToPrint[data[-2]] = {"Total sleep time": str(totalSleepDurationHours)+" hours "+str(totalSleepDurationMin)+" min",  "Avg. mental recovery": round(avgMentalRecovery, 2), "Avg. physical recovery": round(avgPhysicalRecovery, 2),
                             "Avg. movement awakening": round(avgMovementAwakening, 4), "Total sleep cycles": round(totalSleepCycles, 4), "Avg. sleep efficiency": round(avgSleepEfficiency, 2), "Avg. sleep score": round(avgSleepScore, 2)}
# print(dictToPrint)
